...
* */
    ...
    //调用客服接口,发送消息
    sendMessage: function (accessToken, messageData, callback) {
        var messageSendUrl = 'https://api.weixin.qq.com/cgi-bin/message/custom/send?access_token='+accessToken;
        request.post(messageSendUrl)
            .set('Accept', 'application/json')
            .send(messageData)
            .end(function (err, httpResponse) {
                if (httpResponse && httpResponse.status === 200) {
                    ...
                    callback(null, "failed to send message");
                }
            });
    }
